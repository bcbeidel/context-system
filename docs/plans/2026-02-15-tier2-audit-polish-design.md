...
2. **`check_knowledge_base.py --tier2`** -- Runs all triggers, returns `{queue: [...], summary: {...}}`.
...
1. **Run end-to-end against sandbox knowledge base** -- Execute the audit workflow manually against `sandbox/` to identify friction points.
...
- `/dewey:health audit` can be run against the sandbox knowledge base and produces a combined Tier 1 + Tier 2 report
